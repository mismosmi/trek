...
class TrekDatabase {
  ...
  alterRow(rowData, rowId) {
    this.ajaxRequest(
      {operation: 'ALTER', tableName: this.tableName, lastUpdate = this.lastUpdate,
        row: rowId, data: rowData},
      (response) => {
        this.table.updateBody(response.data);
        this.lastUpdate = response.time;
      }
  ...
  deleteRow(rowId) {
    this.ajaxRequest(
      {operation: 'DELETE', tableName: this.tableName, lastUpdate = this.lastUpdate, row: rowId},
      (response) => {
        this.table.updateBody(response.data);
        this.lastUpdate = response.time;
      }
  ...
  editSubmit() {
    ...
    this.editForm.getElementsByTagName('input').forEach( (input) => {
      data[id][input.getAttribute('data-col')] = input.value;
    });
    this.alterRow(data, id);
    this.editDone();
  }
