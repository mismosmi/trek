...
class Database extends Page
{
    ...
    */
    ...
    ) 
    {
        ...
        if (array_key_exists('extensions', $this->dbInfo)) {
            foreach ($this->dbInfo['extensions'] as $extension) {
                if (array_key_exists('js', $extension)) $this->addJs($extension['js']);
                if (array_key_exists('css', $extension)) $this->addCss($extension['css']);
            }
        }
    ...
    public function getScript()
    {
        ...
        foreach ($this->dbInfo['sheets'] as $tableName => $sheet) {
            ...
            if (array_key_exists('type', $sheet) && $sheet['type'] !== 'table') {
                ...
                $sheets .= 
                     "      '$tableName': { title: '$title', type: 'table', columns: [\n";
